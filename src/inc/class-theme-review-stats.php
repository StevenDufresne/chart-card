...
class Theme_Review_Stats extends WP_REST_Controller {
	...
	public function get_uploaded_themes() {
		...
		return $wpdb->get_results(
			...
			FROM  wporg_35_posts p
			JOIN  wporg_35_postmeta pm ON p.ID = pm.post_id AND pm.meta_key = '_author'
			LEFT JOIN wporg_35_term_relationships tr ON p.ID = tr.object_id
			LEFT JOIN wporg_35_term_taxonomy tt ON tr.term_taxonomy_id = tt.term_taxonomy_id AND tt.taxonomy = 'post_tag'
			LEFT JOIN wporg_35_terms t ON tt.term_id = t.term_id
